    ...
    if (transporter) {
      try {
        const info = await transporter.sendMail({
          from: `"WasteWise Dispatch" <${process.env.EMAIL_USER || 'no-reply@wastewise.com'}>`,
          to: req.user.email,
          subject: 'WasteWise: Pickup Scheduled Successfully! 🌍',
          html: `<div style="font-family: Arial, sans-serif; color: #333; max-width: 600px; margin: auto;">
                  <h2 style="color: #10b981;">Hello, EcoHero! 🌿</h2>
                  <p>Incredible work taking action to keep our planet clean! A specialized WasteWise Unit has logged your extraction coordinates.</p>
                  ...
                  <p>Our autonomous units are standing by. We will email you again when the fleet is 'In-Route'. Thank you for using WasteWise!</p>
                </div>`
        });
      ...
      try {
        ...
        if (dbUser) {
          const info = await transporter.sendMail({
            from: `"WasteWise Dispatch" <${process.env.EMAIL_USER || 'no-reply@wastewise.com'}>`,
            to: dbUser.email,
            subject: `WasteWise: Pickup Status Update -> ${status.toUpperCase()}! 🌍`,
            html: `<div style="font-family: Arial, sans-serif; color: #333; max-width: 600px; margin: auto;">
                    <h2 style="color: #10b981;">Hello, EcoHero! 🌿</h2>
