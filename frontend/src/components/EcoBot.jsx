...
const EcoBot = () => {
  ...
  const [messages, setMessages] = useState([
    { sender: 'bot', text: "Hi there! ðŸŒ¿ I'm EcoBot, your friendly green assistant! How can I help you sort your recycling today?" }
  ]);
        ...
        {isOpen && (
          ...
          >
            <div className="bg-zinc-900 border-b border-zinc-800 p-4 text-emerald-400 font-bold flex justify-between items-center">
              <span className="flex items-center gap-2 text-lg font-extrabold"><span className="text-2xl drop-shadow-[0_0_10px_rgba(52,211,153,0.5)] animate-bounce">ðŸ¤–</span> EcoBot</span>
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => setIsOpen(false)} className="hover:text-zinc-200 opacity-50 hover:opacity-100 transition-opacity">âœ–</button>
              </div>
            </div>
      ...
      {!isOpen && (
        <div className="relative group flex items-center justify-center">
          ...
          <div className="absolute right-full mr-4 bg-emerald-900/80 backdrop-blur-md border border-emerald-500/40 text-white text-xs font-bold px-4 py-2 rounded-2xl shadow-xl opacity-0 scale-90 group-hover:opacity-100 group-hover:scale-100 transition-all duration-300 pointer-events-none whitespace-nowrap z-50">
            Hi! I'm EcoBot ðŸŒ¿<br />Need recycling help?
          </div>
          <motion.button
