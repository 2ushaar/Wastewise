...
const Register = () => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      alert('Network Error connecting to WasteWise Server.');
      setLoading(false);
    }
  };
  ...
  const handleGoogleSuccess = async (credentialResponse) => {
    try {
      ...
      alert('Network Error connecting to WasteWise Server via Google.');
    }
  };
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center p-4 overflow-hidden bg-zinc-50 dark:bg-[#09090b] transition-colors duration-500">
      ...
      <motion.div 
        ...
        className="w-full max-w-md bg-white/80 dark:bg-zinc-900/60 rounded-3xl border border-zinc-200 dark:border-zinc-800 shadow-2xl backdrop-blur-xl p-8 relative z-10 min-h-[500px] flex flex-col justify-center"
      >
          ...
          {welcomeMsg ? (
            ...
            <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              ...
              <p className="text-center mt-6 text-zinc-500 dark:text-zinc-400 text-sm">
                Already have an account? <Link to="/login" className="text-emerald-600 dark:text-emerald-500 font-bold hover:text-emerald-500 dark:hover:text-emerald-400 transition-colors">Sign In</Link>
              </p>
            </motion.div>
          )}
        </AnimatePresence>
